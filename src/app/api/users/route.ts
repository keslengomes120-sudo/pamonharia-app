...
// Admin sees everything (permissions = null). Non-admin stores the granted nav hrefs.
// Accepts an array of hrefs or an already-serialized JSON string.
export function serializePermissions(role: string, permissions: unknown): string | null {
  ...
  const hrefs = list.filter((p): p is string => typeof p === "string");
...
export async function POST(req: NextRequest) {
  ...
  const user = await db.user.create({
    data: {
      ...
      password: hashed,
      role,
      permissions: serializePermissions(role, body.permissions),
      active: true,
    },
    select: { id: true, name: true, email: true, role: true, permissions: true, active: true, createdAt: true },
  });
