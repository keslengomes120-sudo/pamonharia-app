  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] md:h-[calc(100vh-0px)]">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-card">
        <div>
          <h1 className="font-bold text-foreground">🤖 Assistente IA</h1>
          {config && (
            <p className="text-xs text-subtle">{providerLabels[config.provider] ?? config.provider} — {config.model}</p>
          )}
        </div>
        <a href="/configuracoes" className="text-xs text-primary hover:underline">Configurar IA</a>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-muted">
        {messages.length === 0 && !loading && (
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground text-sm font-medium">Olá! Sou sua IA especialista em pamonharia.</p>
            <p className="text-subtle text-xs mt-1">Posso analisar suas vendas, CMV, estoque e muito mais.</p>
          </div>
        )}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
            {QUICK_ACTIONS.map((a) => (
              <button key={a} onClick={() => send(a)}
                className="flex-shrink-0 px-3 py-1.5 bg-card border border-border rounded-full text-xs text-muted-foreground hover:border-primary hover:text-primary whitespace-nowrap">
                {a}
              </button>
            ))}
          </div>
      ...
      <div className="px-4 py-3 bg-card border-t border-border">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 bg-muted rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-ring focus:bg-card transition-colors"
          />
          <button
            onClick={() => send()}
            disabled={!input.trim() || loading}
            className="w-12 h-12 bg-primary text-white rounded-2xl flex items-center justify-center disabled:opacity-40 hover:bg-primary-hover transition-colors"
          >
